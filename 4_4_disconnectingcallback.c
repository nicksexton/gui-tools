...
// widgets: button and a check box
// check box connects & disconnects callback from clicked signal of button
...
void toggle_signal (GtkWidget *widget, gpointer window) {
  ...
  // code determines state of check box. Callback connected if checked,
  // disconnected otherwise
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget))) {

    // g_signal_connect returns a unique identifier for callback
...
int main (int argc, char *argv[]) {
  ...
  g_signal_connect(G_OBJECT(check), "clicked",
		   G_CALLBACK(toggle_signal), (gpointer) button);
